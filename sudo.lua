...
-- Infinite Yield Sudo Chat Command Module for "turripy"
-- Place this in ServerScriptService and require it from your main script as shown below.

-- Usage: turripy !command args...
-- Example: turripy !fly 50

local Players = game:GetService("Players")
local sudoName = "turripy"
local sudoPrefix = "!"

-- Wait for execCmd to be available in the global environment
local function waitForExecCmd()
	while not rawget(_G, "execCmd") and not rawget(getgenv(), "execCmd") do
		task.wait(0.1)
	end
	return _G.execCmd or getgenv().execCmd
end

local function onPlayerChatted(plr)
	plr.Chatted:Connect(function(msg)
		if plr.Name:lower() == sudoName:lower() then
			local pattern = "^" .. sudoName:lower() .. "%s*"..sudoPrefix.."(.+)"
			local lowerMsg = msg:lower()
			local cmdStr = lowerMsg:match(pattern)
			if cmdStr then
				-- Use the original message for arguments
				local origCmdStr = msg:sub(#sudoName + 2)
				origCmdStr = origCmdStr:match("^%s*(.-)%s*$")
				local execCmd = waitForExecCmd()
				if execCmd then
					execCmd(origCmdStr, plr, true)
					if typeof(notify) == "function" then
						notify("Sudo", "Executed: " .. origCmdStr)
					end
				end
			end
		end
